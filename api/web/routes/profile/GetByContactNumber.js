...
const APIHandler = (req, res) => {
    ...
    let condition = [
        { "$match": { "contactNumber": req.params.contactNumber } },
        {
            "$project": {
    ...
    userList.Aggregate(condition, (err, result) => {
        ...
        return res({message:req.i18n.__('GtProfileByContactNumber')['200'], data: result }).code(200);
    });
};
...
const response = {
    status: {
        200: {
            message: Joi.any().default(local['GtProfileByContactNumber']['200']), data: Joi.any()
        },
        204: { message: Joi.any().default(local['GtProfileByContactNumber']['204']) },
